...
class Preview extends Disposable {
  ...
  private async getWebviewContents(): Promise<string> {
    ...
    console.log('getting paaviewer.html', settings);
...
<head>
  ...
  <link rel="stylesheet" href="${escapeAttribute(
    this.extensionResource("/media/imageMain.css")
  )}" type="text/css" media="screen" nonce="${nonce}">

  <meta http-equiv="Content-Security-Policy" content="default-src 'self' data: ${
    this.webviewEditor.webview.cspSource
  }; img-src 'self' vscode-resource: blob: data: ${
      this.webviewEditor.webview.cspSource
    }; script-src 'nonce-${nonce}' 'unsafe-eval'; style-src 'self' 'nonce-${nonce}'; object-src 'self';">
...
<body class="container image scale-to-fit loading">
  ...
  <div class="image-load-error">
    ...
    <a href="#" class="open-file-link">${vscode.l10n.t(
      "preview.imageLoadErrorLink",
      "Open file using VS Code's standard text/binary editor?"
    )}</a>
  </div>
  <script nonce="${nonce}">const WASM_URL = "${escapeAttribute(
      this.extensionResource("/media/paa_bg.wasm")
    )}";</script>
  <script src="${escapeAttribute(
    this.extensionResource("/media/paa.js")
  )}" nonce="${nonce}"></script>
  <script src="${escapeAttribute(
    this.extensionResource("/media/imageMain.js")
  )}" nonce="${nonce}"></script>
  <script src="${escapeAttribute(
    this.extensionResource("/media/paaViewer.js")
  )}" nonce="${nonce}"></script>
</body>
</html>`;
  }
