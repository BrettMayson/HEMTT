...
_restrictedModels = [
    "", "\A3\Weapons_f\dummyweapon.p3d", "\A3\Weapons_f\laserTgt.p3d",
    "\A3\Structures_F\Mil\Helipads\HelipadEmpty_F.p3d"
];
_blacklist = ["WeaponHolder", "LaserTarget", "Bag_Base"];
...
_ctrlProgress ctrlSetPosition [safezoneX, safezoneY + safezoneH - _ctrlProgressH, safezoneW, _ctrlProgressH];
...

{
    ...
    if (primaryWeapon _object != "") then {
        ...
        } else { _object switchMove "amovpercmstpsnonwnondnon"; };
    };
    ...
    } else { _object enableSimulation false; };
    ...
    _sphere = if (abs (_pointLowest) > abs (_pointHighest * 2 / 3)) then {
        _sphereNoGround
    } else { _sphereGround };
    ...
    if (_class isKindOf "man" && !(_class isKindOf "animal")) then {
        ...
        _fovStart = if (_camDis < 0.35) then {
            0.4
        } else { 0.1 };
