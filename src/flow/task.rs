...
pub trait Task: objekt::Clone + std::marker::Send + std::marker::Sync {
    ...
    fn parallel(&self, _: &Addon, _: &Report, _: &Project, _: &ProgressBar) -> Result<Report, HEMTTError> {
        unimplemented!()
    }
    fn single(&self, _: Vec<Result<(Report, Addon), HEMTTError>>, _: &Project) -> AddonList {
        unimplemented!()
    }
}
