use std::io::{Cursor, Error, SeekFrom, Write};

use hemtt_io::async_tokio::{ReadExt, WriteExt};
...
#[derive(Default)]
pub struct ReadablePbo<I: AsyncSeekExt + AsyncReadExt + std::marker::Unpin> {
...
impl<I: AsyncSeekExt + AsyncReadExt + std::marker::Unpin + std::marker::Send> ReadablePbo<I> {
    /// Open a PBO
    pub async fn from(input: I) -> Result<Self, Error> {
        let mut pbo = Self {
        ...
        loop {
            let (header, size) = Header::read_async(&mut pbo.input).await?;
            pbo.blob_start += size as u64;
            if header.method() == 0x5665_7273 {
                loop {
                    let s = pbo.input.read_cstring().await?;
                    pbo.blob_start += s.as_bytes().len() as u64 + 1;
                    if s.is_empty() {
                        break;
                    }
                    let val = pbo.input.read_cstring().await?;
        ...
        pbo.input.seek(SeekFrom::Current(1)).await?;
        let mut checksum = vec![0; 20];
        pbo.input.read_exact(&mut checksum).await?;
        pbo.checksum = checksum;
        if let Ok(u) = pbo.input.read(&mut Vec::with_capacity(1)).await {
            if u > 0 {
    ...
    pub async fn gen_checksum(&mut self) -> std::io::Result<Vec<u8>> {
        ...
        if let Some(prefix) = self.extensions.get("prefix") {
            ...
            headers.write_cstring(prefix.as_bytes()).await?;
        }
        ...
        for (key, value) in &self.extensions {
            ...
            headers.write_cstring(key.as_bytes()).await?;
            headers.write_cstring(value.as_bytes()).await?;
        }
        ...
        for header in &files {
            let cursor = self.retrieve(header.filename()).await.unwrap();
    ...
    pub async fn retrieve(&mut self, filename: &str) -> Option<Cursor<Vec<u8>>> {
        ...
        for h in &self.headers {
            if h.filename().to_lowercase() == filename.to_lowercase() {
                let mut buffer: Vec<u8> = vec![0; h.size() as usize];
                self.input.read_exact(&mut buffer).await.unwrap();
