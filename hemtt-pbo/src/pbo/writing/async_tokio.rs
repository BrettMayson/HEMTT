...
impl<I: AsyncSeekExt + AsyncReadExt + std::marker::Unpin + std::marker::Send> WritablePbo<I> {
    ...
    pub async fn add_file<S: Into<String>>(
        &mut self,
        filename: S,
        mut file: I,
        ...
        let size = file.seek(SeekFrom::End(0)).await? as u32;
    ...
    pub async fn add_file_header<S: Into<String>>(
        &mut self,
        filename: S,
        mut file: I,
        header: Header,
    ...
    pub async fn retrieve_file<S: Into<String>>(
        ...
        if self.files.contains_key(filename) {
            ...
            let mut buffer: Box<[u8]> =
                vec![0; data.seek(SeekFrom::End(0)).await? as usize].into_boxed_slice();
    ...
    /// Write the PBO file
    pub async fn write<O: Write>(&mut self, output: &mut O) -> Result<()> {
        ...
        for header in &files_sorted {
            ...
            let cursor = self.retrieve_file(header.filename()).await?.unwrap();
    ...
    pub async fn checksum(&mut self) -> Result<Vec<u8>> {
        let mut headers: Cursor<Vec<u8>> = Cursor::new(Vec::new());
...
mod tests {
    ...
    async fn empty_pbo() {
        ...
        pbo.write(&mut Cursor::new(&mut buffer)).await.unwrap();
        assert_eq!(
            pbo.checksum().await.unwrap(),
            vec![
