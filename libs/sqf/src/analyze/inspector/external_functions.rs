...
impl Inspector<'_> {
    ...
    pub fn external_function_call(
        ...
        if let Some(lhs) = lhs {
            self.external_check_code_usage(lhs, &ext_func_lower);
        }

        // check if we should analyze this function, either by wildcard or prefix match
        let check_function_calls = self.database.inspector_config()?.check_function_calls();
        if !check_function_calls
            .iter()
            .any(|prefix| prefix == "*" || ext_func_lower.starts_with(&prefix.to_lowercase()))
        {
            return None;
        }
    ...
    fn external_check_code_usage(&mut self, lhs: &IndexSet<GameValue>, ext_func_lower: &str) {
        for possible in lhs {
            match possible {
                GameValue::Code(Some(statements)) if ext_func_lower == "cba_fnc_directcall" => {
