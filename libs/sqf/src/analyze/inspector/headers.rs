...
pub(crate) fn extract_from_header(
    ...
    let (re_header, re_arg_line) = get_regex(inspector_config?)?;
    for (path, source) in processed.sources() {
        ...
        {
            match match_header(source, funcname, &re_header, &re_arg_line) {
...
fn get_regex_cba() -> (Regex, Regex) {
    ...
    }).clone(), RE_ARG_LINE.get_or_init(|| {
        Regex::new(r"^    _(?<info>.*)").expect("regex ok") // 4 spaces before _
    }).clone())
}
#[must_use]
fn get_regex_ace() -> (Regex, Regex) {
    static RE_HEADER: OnceLock<Regex> = OnceLock::new();
    static RE_ARG_LINE: OnceLock<Regex> = OnceLock::new();
    (RE_HEADER.get_or_init(|| {
...
#[must_use]
fn get_regex(inspector_config: &InspectorOptions) -> Option<(Regex, Regex)> {
    let header_regex = inspector_config.header_regex();
    let header_line_regex = inspector_config.header_line_regex();

    match header_regex.to_lowercase().as_str() {
        "" => None,
        "ace" => Some(get_regex_ace()),
        "cba" => Some(get_regex_cba()),
        _ => {
            let header_re = Regex::new(header_regex).ok()?;
            let line_re = Regex::new(header_line_regex).ok()?;
            Some((header_re, line_re))
        }
    }
}
...
mod tests {
    ...
    pub fn test_header() {
        let options =
            hemtt_common::config::InspectorOptions::default().with_header_regex("ace".to_string());
        let (re_header, re_arg_line) = super::get_regex(&options).expect("regex for ace");
        let source = r#"
/*
 * Arguments:
 * 0: "test" <ARRAY>
 ...
 */
"#;
        let h = super::match_header(
            source,
            "fnc_test_header".to_lowercase().as_str(),
            &re_header,
            &re_arg_line,
        );
        println!("Header: {h:?}");
