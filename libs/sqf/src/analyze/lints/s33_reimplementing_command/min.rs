...
pub fn check(target: &Expression, processed: &Processed, config: &LintConfig) -> Vec<Arc<dyn Code>> {
        ...
        && let Some((left_expr, right_expr)) = check_select_pattern(array, condition)
    {
        ...
        codes.push(Arc::new(CodeS33ReimplementingCommandMin::new(
            target.full_span(),
            left_text,
            right_text,
            processed,
    ...
    // Check if then and else branches match the min pattern
    match comparison_op {
...
// Check for select pattern: [x, y] select (x > y) -> x min y
...
fn check_select_pattern<'a>(array: &'a Expression, condition: &'a Expression) -> Option<(&'a Expression, &'a Expression)> {
    ...
    // This is equivalent to: min(x, y)
    if matches!(comparison_op, BinaryCommand::Greater | BinaryCommand::GreaterEq)
...
pub struct CodeS33ReimplementingCommandMin {
    span: Range<usize>,
    left: String,
    right: String,
    severity: Severity,
...
impl Code for CodeS33ReimplementingCommandMin {
    ...
    fn message(&self) -> String {
        String::from("code can be replaced with `min`")
    }
    ...
    fn suggestion(&self) -> Option<String> {
        Some(format!("{} min {}", self.left, self.right))
    }
...
impl CodeS33ReimplementingCommandMin {
    #[must_use]
