    ...
    fn run(
        ...
        for issue in target.issues() {
            if let Issue::MismatchedTypes { span, command } = issue {
                errors.push(Arc::new(CodeS38BranchTypesMismatch::new(
                    span.to_owned(),
                    command.clone(),
                    config.severity(),
                    processed,
...
pub struct CodeS38BranchTypesMismatch {
    span: Range<usize>,
    command: String,
    severity: Severity,
...
impl Code for CodeS38BranchTypesMismatch {
    fn ident(&self) -> &'static str {
        "L-S37"
    }
...
impl CodeS38BranchTypesMismatch {
    #[must_use]
    pub fn new(
        span: Range<usize>,
        command: String,
