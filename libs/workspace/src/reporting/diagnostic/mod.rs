...
impl Diagnostic {
    ...
    pub fn to_string(&self, files: &WorkspaceFiles) -> String {
        ...
        let ansi = String::from_utf8(buffer.into_inner())
            .expect("utf8")
            .replace("\u{1b}[34m", "\u{1b}[36m");
        if hemtt_common::ansi::ansi_supported() {
            ansi
        } else {
            String::from_utf8(strip_ansi_escapes::strip(ansi.as_bytes()))
                .expect("stripping ansi should succeed")
        }
    }
